...
{
    // ==================== CLASS CHỨC NĂNG SỬA MÔN HỌC (BLL) ====================
    public class ChucNangSuaMonHoc
    {
        /// <summary>
        ...
        /// <summary>
        /// Tìm vị trí môn học theo mã
        /// </summary>
        private int TimViTriMonHoc(List<ThongTinMonHoc> danhSach, string maMonHoc)
        {
            bool maRong = KiemTraChuoiRong(maMonHoc);
            if (maRong)
                return -1;

            for (int i = 0; i < danhSach.Count; i++)
            {
                string maMHHienTai = danhSach[i].MaMonHoc;
                bool khopMa = SoSanhChuoiKhongPhanBietHoaThuong(maMHHienTai, maMonHoc);
        ...
        /// <summary>
        /// Kiểm tra dữ liệu môn học có hợp lệ không
        /// </summary>
        private bool KiemTraDuLieuHopLe(ThongTinMonHoc mh)
        {
            bool maRong = KiemTraChuoiRong(mh.MaMonHoc);
            if (maRong)
                return false;

            bool tenRong = KiemTraChuoiRong(mh.TenMonHoc);
            if (tenRong)
                return false;
        ...
        /// </summary>
        public bool SuaMonHoc(List<ThongTinMonHoc> danhSach, string maMonHoc, ThongTinMonHoc monHocMoi)
        {
            if (danhSach == null || monHocMoi == null)
                return false;

            int viTri = TimViTriMonHoc(danhSach, maMonHoc);

            if (viTri == -1)
                return false;

            bool duLieuHopLe = KiemTraDuLieuHopLe(monHocMoi);

            if (!duLieuHopLe)
                return false;

            danhSach[viTri] = monHocMoi;
